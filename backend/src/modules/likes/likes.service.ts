...
export class LikesService {
  ...
  async addLike(userId: string, imageId: string): Promise<LikeStatusDto> {
    ...
    const alreadyLiked = await this.likesRepository.existsByUserAndImage(
      userId,
      imageId,
    );
    if (alreadyLiked) {
  ...
  async getLikedImages(
    ...
    const [images, total] = await this.likesRepository.findLikedImagesByUserId(
      userId,
      page,
      limit,
    );
